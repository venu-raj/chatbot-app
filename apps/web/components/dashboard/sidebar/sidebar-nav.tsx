...
// import { NavWordmark } from "@workspace/ui/components/nav-wordmark";
                        ...
                        {title &&
                          (typeof title === "string" ? (
                            ...
                            >
                              ...
                              <span className="text-content-emphasis text-lg font-semibold text-black">
                                {title}
                              </span>
                            </TitleContainer>
                          ) : (
                        ...
                        <div className="flex flex-col gap-8">
                          {content.map(({ name, items }, idx) => (
                            <div key={idx} className="flex flex-col gap-0.5">
                              {name && (
                                <div className="mb-2 pl-3 text-sm font-medium text-muted-foreground">
                                  {name}
                                </div>
                              )}
                              {items.map((item) => (
    ...
    <div>
      <Link
        ...
        className={cn(
          ...
          "outline-none focus-visible:ring-2 text-black/75 font-medium focus-visible:ring-black/50",
          isActive && !items
      ...
      >
        ...
        <span className="ml-2 flex items-center gap-2">
          ...
          {items && (
            <ChevronDown className="size-3.5 transition-transform duration-75 group-data-[active=true]:rotate-180" />
          )}
          {item.arrow && (
...
export function Area({
  ...
  return (
    <AnimatePresence mode="wait">
      {visible && (
        <motion.div
          key={direction} // Important: use a unique key for each area
          initial={{
            opacity: 0,
