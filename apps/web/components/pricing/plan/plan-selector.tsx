...
import { ADVANCED_PLAN, BUSINESS_PLAN, PRO_PLAN, cn } from "@workspace/utils";
...
export const PLAN_SELECTOR_PLANS = [
  PRO_PLAN,
  BUSINESS_PLAN,
  ADVANCED_PLAN,
  PRO_PLAN,
        ...
        >
          {PLAN_SELECTOR_PLANS.map((plan) => (
            <div
              key={plan.name}
              className={cn(
                plan.name === BUSINESS_PLAN.name
                  ? "flex h-full flex-col border-y border-l border-neutral-200 bg-white bg-gradient-to-b from-orange-50 to-40% p-5 last:border-r max-lg:translate-x-[calc(-1*var(--index)*(100%+32px))] max-lg:opacity-0 max-lg:transition-[transform,opacity]"
            ...
            >
              <div className="flex items-center justify-between gap-2">
                ...
                {recommendedPlan
                  ? recommendedPlan === plan.name.toLowerCase() && (
                      <Badge
                        variant="sky"
                        className="px-1.5 text-[0.5rem] uppercase"
                      >
                        Recommended
                      </Badge>
                    )
              ...
              <div className="mt-1 text-start text-base font-medium text-neutral-400">
                <NumberFlow
